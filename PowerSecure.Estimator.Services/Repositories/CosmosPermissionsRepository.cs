...
namespace PowerSecure.Estimator.Services.Repositories
{
    public class CosmosPermissionsRepository : IPermissionsRepository
    {
        ...
        {
            var documentQuery = _dbClient.CreateDocumentQuery<Permissions>(UriFactory.CreateDocumentCollectionUri(databaseId: _databaseId, collectionId: _collectionId), new FeedOptions { EnableCrossPartitionQuery = false })
                .AsDocumentQuery();
            ...
            {
                foreach (Permissions item in await documentQuery.ExecuteNextAsync())
                {
                    ids.Add(item.Id);
                }
            }
            ...
            foreach (var child in jToken.Children())
            {
                ...
                var item = child.ToObject<Permissions>();
